<!DOCTYPE html>
<html class="light" lang="<?php echo e(str_replace('_', '-', app()->getLocale() ?? 'en')); ?>">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo $__env->yieldContent('title', 'DevTrack'); ?></title>
    ...
    <aside class="hidden md:flex flex-col fixed top-0 left-0 h-screen w-64 p-4 gap-2 bg-white border-r border-outline-variant font-sans text-sm font-medium z-50">
        ...
        <div class="flex items-center gap-3 px-2 py-4 mb-4 bg-surface-container-low rounded-xl">
            ...
            <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($user->name)); ?>&background=random"
                 alt="<?php echo e($user->name); ?>" class="w-10 h-10 rounded-full object-cover">
            <div>
                <div class="text-on-surface font-bold text-sm"><?php echo e($user->name); ?></div>
                <div class="text-outline text-xs"><?php echo e($user->email); ?></div>
            </div>
            <?php endif; ?>
        </div>
...
<nav class="flex-1 space-y-1 overflow-y-auto">
            ...
            <?php $__currentLoopData = $navLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                ?>
                <a href="<?php echo e($link['href']); ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all transform <?php echo e($isActive ? 'bg-primary text-white shadow-sm' : 'text-outline hover:bg-surface-container hover:text-primary hover:translate-x-1'); ?>">
                    <span class="material-symbols-outlined text-xl"><?php echo e($link['icon']); ?></span>
                    <span class="font-bold"><?php echo e($link['label']); ?></span>
                </a>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden md:ml-64">
        ...
        <header class="sticky top-0 z-40 flex justify-between items-center w-full px-6 h-16 bg-white border-b border-outline-variant shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-xl font-bold tracking-tight text-on-surface"><?php echo $__env->yieldContent('page-title'); ?></h1>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <?php $user = auth()->user(); ?>
                <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($user->name)); ?>&background=random" 
                     alt="<?php echo e($user->name); ?>" class="w-8 h-8 rounded-full border border-outline-variant object-cover">
                <?php endif; ?>
            </div>
        </header>
        ...
        <nav class="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-2 py-3 pb-safe bg-white/80 backdrop-blur-md border-t border-outline-variant">
            <a href="/dashboard" class="flex flex-col items-center gap-1 text-outline hover:text-primary transition-all <?php echo e(request()->is('dashboard') ? 'text-primary' : ''); ?>">
                <span class="material-symbols-outlined">dashboard</span>
                <span class="text-[10px] font-bold">Dash</span>
            </a>
            <a href="/projects" class="flex flex-col items-center gap-1 text-outline hover:text-primary transition-all <?php echo e(request()->is('projects*') || request()->is('project*') ? 'text-primary' : ''); ?>">
                <span class="material-symbols-outlined">folder</span>
                <span class="text-[10px] font-bold">Proj</span>
            </a>
            <a href="/tasks" class="flex flex-col items-center gap-1 text-outline hover:text-primary transition-all <?php echo e(request()->is('tasks') ? 'text-primary' : ''); ?>">
            ...
            <a href="/archives" class="flex flex-col items-center gap-1 text-outline hover:text-primary transition-all <?php echo e(request()->is('archives') ? 'text-primary' : ''); ?>">
...
<?php /**PATH C:\Users\jdira\Herd\devtrack\resources\views/layouts/app.blade.php ENDPATH**/ ?>