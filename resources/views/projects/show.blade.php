            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-outline font-bold text-xs uppercase tracking-tighter tracking-widest">Project #{{ $project->id }}</span>
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div class="lg:col-span-2 space-y-6">
            ...
            @forelse($project->tasks as $task)
                <div class="bg-white border border-outline-variant p-6 rounded-xl shadow-sm hover:shadow-md hover:translate-x-1 transition-all group">
                    <div class="flex justify-between items-start mb-6">
                        <div class="space-y-3">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @if($task->priority)
                                    <span class="{{ $task->priority === 'high' ? 'bg-error/10 text-error' : ($task->priority === 'medium' ? 'bg-tertiary/10 text-tertiary' : 'bg-surface-container text-outline') }} px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">
                                        {{ $task->priority }}
                                    </span>
                                @endif
                            </div>
                        </div>
                        <a href="{{ route('tasks.edit', $task->id) }}" class="text-outline hover:text-primary transition-colors p-1">
                    ...
                    <div class="flex items-center justify-between border-t border-outline-variant/30 pt-4">
                        <div class="flex -space-x-2">
                            @if($task->assignee)
                                <img class="h-8 w-8 rounded-full border-2 border-white ring-1 ring-outline-variant" src="https://ui-avatars.com/api/?name={{ urlencode($task->assignee->name) }}" alt="{{ $task->assignee->name }}">
                            @else
                                <div class="h-8 w-8 rounded-full border-2 border-white bg-surface-container flex items-center justify-center text-[10px] font-bold text-outline ring-1 ring-outline-variant">?</div>
                            @endif
                        </div>
            ...
            @empty
                <div class="bg-white border border-outline-variant p-8 rounded-xl text-center">
                    <div class="w-16 h-16 bg-surface-container-low rounded-full flex items-center justify-center text-outline mb-4 mx-auto">
                ...
                <div class="space-y-5">
                    @forelse($project->members as $member)
                        <div class="flex items-center justify-between group cursor-pointer">
                            <div class="flex items-center gap-3">
                                <img class="w-10 h-10 rounded-full border border-outline-variant shadow-sm group-hover:ring-2 group-hover:ring-primary/20 transition-all" src="https://ui-avatars.com/api/?name={{ urlencode($member->name) }}" alt="{{ $member->name }}">
                                <div>
                                    <div class="text-sm font-bold text-on-surface group-hover:text-primary transition-colors">{{ $member->name }}</div>
                                    <div class="text-[10px] text-outline font-medium">{{ $member->pivot->role ?? 'Member' }}</div>
                                </div>
                            </div>
