...
@section('content')
    <!-- Header Section -->
    <div
        class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-lg animate-in fade-in slide-in-from-left duration-500">
        <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
        <!-- Main Focus Project -->
        <div
            class="col-span-1 lg:col-span-2 bg-white border border-outline-variant rounded-xl p-6 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
            <div class="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full -mr-16 -mt-16 blur-2xl"></div>

            <div class="flex justify-between items-start mb-6">
                <div class="flex items-center gap-4">
                    <div
                        class="w-12 h-12 bg-secondary-container/30 text-secondary rounded-lg flex items-center justify-center">
                        <span class="material-symbols-outlined text-[28px]">database</span>
                    </div>
                    <div>
                        <h3 class="text-xl font-bold text-on-surface group-hover:text-primary transition-colors">CloudScale
                            Architecture</h3>
                        <p class="text-xs text-outline font-medium uppercase tracking-wider">Internal Infrastructure</p>
                    </div>
                </div>
                <span
                    class="px-3 py-1 bg-secondary/10 text-secondary text-[10px] rounded-md font-bold uppercase tracking-widest">On
                    Track</span>
            </div>

            <p class="text-sm text-on-surface-variant mb-8 line-clamp-2 leading-relaxed">
                Rebuilding the core microservices architecture to support 10x concurrent user growth and implementing global
        ...
        <div class="bg-white border border-outline-variant rounded-xl p-6 shadow-sm hover:shadow-md transition-all group">
            <div class="flex justify-between items-start mb-6">
                ...
                <span
                    class="px-2 py-1 bg-tertiary/10 text-tertiary text-[10px] rounded-md font-bold uppercase tracking-widest">Priority</span>
            </div>
            <h3 class="text-xl font-bold text-on-surface mb-2 group-hover:text-primary transition-colors">Auth2.0 Migration
            </h3>
            <p class="text-sm text-on-surface-variant mb-6 line-clamp-2">Updating identity providers and securing legacy API
                endpoints.</p>
            <div class="space-y-4">
                <div class="flex justify-between items-center text-xs font-bold uppercase tracking-tight">
        ...
        <div class="bg-white border border-outline-variant rounded-xl p-6 shadow-sm hover:shadow-md transition-all group">
            ...
            <p class="text-sm text-on-surface-variant mb-6 line-clamp-2">Standardizing component library for cross-platform
                apps.</p>
            <div class="space-y-4">
                <div class="flex justify-between items-center text-xs font-bold uppercase tracking-tight">
                    <span class="text-outline">Tasks</span>
        ...
        <!-- Add New Placeholder -->
        <div
            class="border-2 border-dashed border-outline-variant rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-white hover:border-primary transition-all group cursor-pointer group">
            <div
                class="w-14 h-14 rounded-full bg-surface-container-low flex items-center justify-center text-outline group-hover:bg-primary/10 group-hover:text-primary mb-4 transition-all">
                <span class="material-symbols-outlined text-3xl">add_circle</span>
            </div>
            <p class="text-lg font-bold text-on-surface group-hover:text-primary transition-colors">Initiate Project</p>
            <p class="text-xs text-outline px-4 mt-2 leading-relaxed">Start a new workflow and assign your core development
                team.</p>
        </div>
    </div>
@endsection
